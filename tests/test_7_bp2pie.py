...
def test_parse_arguments(monkeypatch):
    ...
    test_args = ["h2o", "bp2pie", "-f", "tests/test_data/bp_output.txt", "-s", "tests/test_data/species_2.tre", "-od", "tests/test_data/other_output", "-p","-n", "testrun"]
    ...
    assert captured_args.consensus_tree_file == "tests/test_data/species_2.tre"
    assert captured_args.output_directory == "tests/test_data/other_output"
    assert captured_args.pie_option == True
    assert captured_args.run_name == "testrun"
    ...
    assert captured_args.consensus_tree_file == None
    assert captured_args.run_name == None
...
def test_bp2pie():
    ...
    args = argparse.Namespace(
        bp_output_file="tests/test_data/bp_output.txt,tests/test_data/bp_output2.txt",
        output_directory="tests/test_data/other_output",
        consensus_tree_file="tests/test_data/species_2.tre",
        pie_option=True,
        run_name=None
    )
    bp2pie.main(args)
    ...
    with open("tests/test_data/other_output/bp_consensus_tree_numbered.tre","r") as f:
...
def test_bp2pie_2():
    args = argparse.Namespace(
        ...
        output_directory="tests/test_data/other_output",
        consensus_tree_file=None,
        pie_option=False,
        run_name="test"
    )
    bp2pie.main(args)
    with open("tests/test_data/other_output/bp_consensus_tree_numbered_test.tre","r") as f:
        assert f.readline() == "(o:0.0,((e:0.0,f:0.0)2:0.0,((a:0.0,b:0.0)4:0.0,(c:0.0,d:0.0)5:0.0)3:0.0)1:0.0)0:0.0;\n"
    with open("tests/test_data/other_output/gokstad_pie_test.tre","r") as f:
